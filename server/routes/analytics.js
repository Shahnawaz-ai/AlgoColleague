...
router.get('/overview', async (req, res) => {
  try {
    ...
    const postsThisWeekRow = await dbGet(
      "SELECT COUNT(*) as count FROM posts WHERE status = 'published' AND published_at >= ?", weekAgo
    );
    const postsThisWeek = postsThisWeekRow ? postsThisWeekRow.count : 0;

    const monthAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();
    const postsThisMonthRow = await dbGet(
      "SELECT COUNT(*) as count FROM posts WHERE status = 'published' AND published_at >= ?", monthAgo
    );
